...
<head>
		...
		<script> 
	        $(function(){
        		 var allData=[];
        		 var index = 1;
        		 <c:forEach var="testCase" items="${itmsTestCaseList}">　              
	     		     var data="<tr>"+  
	     		     "<td>"+"${testCase.id}"+"</td>"+  
	     		     "<td>"+"${testCase.name}"+"</td>"+  
	     		    // "<td>"+"${itmsTestCase.number}"+"</td>"+
	     		   //  "<td>"+"${itmsTestCase.status}"+"</td>"+
	     		   //  "<td>"+"${itmsTestCase.version}"+"</td>"+
	     		  //   "<td>"+"${itmsTestCase.writer}"+"</td>"+
	     		  "<td><input id='script${testCase.id}' value='${testCase.script}'></input></td>" +
	     		  "<td><input id='param${testCase.id}' value='${testCase.param}'></input></td>" +
	     		  "<td><input id='ip${testCase.id}' value='${testCase.ip}'></input></td>" +
	     		     "<td><div class='dropdown'>" + 
	     		 		...
	     		 		"<li role='presentation'><a role='menuitem' tabindex='-1' href='javascript:void(0)' onclick='saveTestCase(${testCase.id})'>保存</a></li>"+
	     		 		"<li role='presentation'><a href='javascript:void(0)' onclick='execTestCase(${testCase.id})'>执行</a></li></ul>"+			
	     		 	"</div></td>" +
	     		 	"<td><a id='status${testCase.id}'></a></td></tr>";  
	     		     allData.push(data);
        		 </c:forEach>
        		...
        		var Count = allData.length;//记录条数  
				 ...
				 allTasks = {};
				<c:forEach var="testTask" items="${itmsTestTaskList}">
				var task = {};
				task.id = "${testTask.id}";
				task.status = "${testTask.taskStatus}";
				allTasks["${testTask.id}"] = task;
				</c:forEach>
				taskIds = Object.keys(allTasks);
				for(var id=0; id<taskIds.length; id++){
					var taskId = taskIds[id];
					var task = allTasks[taskId];
					//alert("id: " + taskId + "; taskStatus: " + task.status);
					$("#status" + taskId).html(task.status);
	        ...
	        function flushStatusOfTasks(){
	        	...
	        	$.ajax({
	        		...
	        		success: function(data){
	        			...
	        			$("#debugInfo").html("getStatus 成功了！ " + data);
	        			var sts = JSON.parse(data);
	        			for(var i=0; i<sts.length; i++){
	        				$("#status" +  sts[i].id).html(sts[i].status);
	        				if(sts[i].status != "RUNNING"){
	        ...
	        function saveTestCase(id){
	        	var script =$('#script'+id).val();
	        	var param = $('#param' + id).val();
	        	var ip = $('#ip' + id).val();
	        ...
	        function execTestCase(id){
	        	$.ajax({
	        		...
	        		success: function(data){
	        			...
	        			alert("execute 成功了！" + data + "; " + ret);
	        			if(ret){
		        			$("#status" + id).html("RUNNIG");
		        			runningTasks[id] = '';		
	        			}
	        		},
	        	});
	       ...
	       function search(){
       	    ...
       	    /* alert(content); */
       	    var urlPath =  "${pageContext.request.contextPath}" + "/testCase.do/testCaseSearch?content=" +	content;		
   		        		...
   		        		for(var i=0;i < searchCaseList.length;i++){
   		        			var data="<tr>"+  
   			     		     ...
   			     		     "<td><div class='dropdown'>" + 
   			     		 	    "<button type='button' class='btn dropdown-toggle' id='dropdownMenu1' data-toggle='dropdown'>操作<span class='caret'></span></button>" + 
   			     		 	    "<ul class='dropdown-menu' role='menu' aria-labelledby='dropdownMenu1' style='min-width:100px;'>" +
   			     		 		"<li role='presentation'><a role='menuitem' tabindex='-1' href='${pageContext.request.contextPath}/testCase.do/edit?id=" + searchCaseList[i].id +"'>编辑</a></li>"+
	        ...
	        function delete_confirm(id,index) {
	        	...
	        	if (r==true)
        	    {
	        	    ...
	        	    var urlPath =  "${pageContext.request.contextPath}" + "/testCase.do/delete?id=" +	testCaseId;		
	    		        		...
	    		        		for(var i=0;i < caseList.length;i++){
	    		        			var data="<tr>"+  
	    			     		     ...
	    			     		     "<td><div class='dropdown'>" + 
	    			     		 	    "<button type='button' class='btn dropdown-toggle' id='dropdownMenu1' data-toggle='dropdown'>操作<span class='caret'></span></button>" + 
	    			     		 	    "<ul class='dropdown-menu' role='menu' aria-labelledby='dropdownMenu1' style='min-width:100px;'>" +
	    			     		 		"<li role='presentation'><a role='menuitem' tabindex='-1' href='${pageContext.request.contextPath}/testCase.do/edit?id="+ caseList[i].id +"'>编辑</a></li>"+
